...
@Tag(name = "01. Festival", description = "축제 정보 및 검색 API")
public interface FestivalControllerDocs {
    ...
    ResponseEntity<ListResponseDTO<FestivalDTO.FestivalSummary>> getFestivals(String keyword, int page, int size);

    @Operation(summary = "축제 상세 조회 API")
    @GetMapping("/api/v1/festivals/{festivalId}")
    ResponseEntity<FestivalDTO.FestivalDetail> getFestivalDetail(Long festivalId);
}